...
- Judge Preview base alias: [https://codex-merch.vercel.app](https://codex-merch.vercel.app)
...
- Optional production checkout: [https://codex-merch.vercel.app](https://codex-merch.vercel.app) — fan-made, unofficial merch; owner-gated, delivery to CH/US, and no purchase required to judge the project
...
- Support contact for judges: use the Contact page on the live storefront.

## Access model

- Application login required: **No**
- Payment mode: **Disabled in the submitted Preview. The separate production mode accepts live payment only when owner approvals pass and public readiness reports `paymentMode: live`**
- External fulfillment: **Disabled in the submitted Preview. A paid production order creates one unconfirmed Printful draft for manual review; the required judge path creates none**
...
6. Checkout is not part of evaluation. If the owner has separately activated production checkout, the signed physical product can be purchased; it is fan-made, unofficial merchandise and is never required for testing or evaluation.

## Expected limitations

- Payments and fulfillment remain outside the submitted Build Week Preview. Owner-gated production checkout is separate; `/api/readiness` proves configuration readiness, while a controlled live order is required to prove the complete payment and fulfillment handoff.
...
- [x] Confirm canonical checkout fails closed unless the owner approvals, signed product, provider mappings, live payment mode, database, webhook, and fulfillment dependencies all pass.
