...
- Country: Switzerland
- Judge Preview: public [canonical storefront](https://codex-merch.vercel.app); no account or token is required
...
| Stripe, Neon, Inngest, and Printful draft purchase path | Implemented as an owner-gated public checkout outside the required Preview | Unit coverage verifies live-mode enforcement, hosted terms consent, expiry, server-side pricing, webhook signatures, immutable order snapshots, idempotency, and fail-closed configuration; a read-only U.S. Printful estimate passed for the exact synced variant | Canonical readiness remains the configuration source of truth. Do not claim a working live payment until it returns HTTP 200 with `paymentMode: live` and a controlled live order completes end to end; the free Preview never requires purchase |
...
- The canonical checkout is a separate, owner-gated production mode. It exposes only the signed product, accepts CH/US delivery, requires every live payment and operational approval before any order or Stripe side effect, and keeps `PRINTFUL_AUTO_CONFIRM=false`. It is fan-made and not official OpenAI merchandise. Judge evaluation remains free through the Preview even if checkout is closed.
