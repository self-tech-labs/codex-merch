...
| Exact Printful sync-ID recheck | **Open — launch gate** | The verifier now compares local sync IDs `5338615120`, `5338615121`, and `5338615122` with the live response. Rerun it successfully after deployment; the final local attempt was prevented before execution by the desktop approval service, so this stronger assertion is not marked complete. |
